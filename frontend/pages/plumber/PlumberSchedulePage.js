...
function formatDateTime(dateTimeString) {
  ...
  const endTime = formatTime(new Date(new Date(dateTimeString).getTime() + 60000)); // Add one minute for end time
...
export default function PlumberSchedulePage() {
  ...
  return (
    <View>
      ...
      <Logo text="My Schedule" />
      <FlatList 
        data={transactions}
        keyExtractor={({ id }) => id}
        renderItem={({ item }) => (
          <Pressable className="bg-white rounded-lg shadow-md m-2 p-4" onPress={() => navigateToIssuePage(item.id)}>
            <Image source={{ uri: item.media }} className="h-32 w-full rounded" resizeMode="cover"/>
            <Text className="font-bold text-lg">{item.title}</Text>
            <Text>{item.description}</Text>
            <Text className="mt-3">Address: {item.address}</Text>
            <Text>Date of Appointment: {formatDateTime(item.meetingDate)}</Text>
